...
namespace helengine::gamecube {
    ...
    GameCubeAudioBackend::GameCubeAudioBackend()
        : ActiveVoicesById()
        , BusGainsById()
        , PausedBusIds() {
    ...
    int32_t GameCubeAudioBackend::Play(::AudioAsset* asset, ::AudioPlaybackRequest* request) {
        if (asset == nullptr) {
            throw std::invalid_argument("asset");
        }
        if (asset->SampleRate <= 0) {
            throw std::runtime_error("GameCube audio playback requires a positive sample rate.");
        }
        if (asset->EncodingFamilyId != "pcm-streamed") {
            throw std::runtime_error("GameCube audio playback currently requires shared pcm-streamed assets.");
        }
        if (asset->EncodedBytes == nullptr || asset->EncodedBytes->Length <= 0 || asset->EncodedBytes->Data == nullptr) {
            throw std::runtime_error("GameCube audio playback requires one non-empty encoded payload.");
        }
        if ((asset->EncodedBytes->Length % static_cast<int32_t>(sizeof(std::int16_t) * asset->Channels)) != 0) {
            throw std::runtime_error("GameCube audio playback requires 16-bit PCM sample alignment.");
        }

        const int32_t voiceFormat = ResolveVoiceFormat(asset);
        const int32_t slot = AcquireVoiceSlot();
        if (slot < 0) {
            throw std::runtime_error("GameCube audio playback could not reserve one ASND voice.");
        }
        ...
        if (audioBuffer == nullptr) {
            throw std::runtime_error("GameCube audio playback could not allocate one aligned ASND buffer.");
        }
        ...
        if (playResult != SND_OK) {
            ...
            throw std::runtime_error("GameCube audio playback failed to queue the ASND voice.");
        }
    ...
    void GameCubeAudioBackend::Stop(int32_t voiceId) {
    ...
    void GameCubeAudioBackend::SetBusGain(std::string busId, float gain) {
    ...
    void GameCubeAudioBackend::SetBusPaused(std::string busId, bool paused) {
    ...
    bool GameCubeAudioBackend::IsPlaying(int32_t voiceId) {
        auto voiceIterator = ActiveVoicesById.find(voiceId);
        if (voiceIterator == ActiveVoicesById.end()) {
            return false;
        }

        const s32 status = ASND_StatusVoice(voiceIterator->second.Slot);
        return status != SND_INVALID && status != SND_UNUSED;
    }

    void GameCubeAudioBackend::Update() {
    ...
    std::string GameCubeAudioBackend::NormalizeBusId(std::string busId) {
        if (busId.empty()) {
            return "master";
        }
    ...
    float GameCubeAudioBackend::ClampGain(float gain) {
    ...
    int32_t GameCubeAudioBackend::ConvertGainToVolume(float gain) {
        return static_cast<int32_t>(ClampGain(gain) * static_cast<float>(MAX_VOLUME));
    }

    int32_t GameCubeAudioBackend::ResolveVoiceFormat(const ::AudioAsset* asset) {
        if (asset == nullptr) {
            throw std::invalid_argument("asset");
        }
        ...
        switch (asset->Channels) {
            ...
            default:
                throw std::runtime_error("GameCube audio playback currently supports only mono or stereo 16-bit PCM assets.");
        }
    }

    int32_t GameCubeAudioBackend::AcquireVoiceSlot() const {
        return ASND_GetFirstUnusedVoice();
    }

    float GameCubeAudioBackend::ResolveCombinedGain(const std::string& busId, float baseGain) const {
    ...
    bool GameCubeAudioBackend::IsBusPaused(const std::string& busId) const {
        return PausedBusIds.contains("master") || PausedBusIds.contains(busId);
    }

    void GameCubeAudioBackend::ApplyVoicePlaybackState(ActiveVoiceState& voice) {
        ...
        const int32_t volume = ConvertGainToVolume(ResolveCombinedGain(voice.BusId, voice.BaseGain));
        ASND_ChangeVolumeVoice(voice.Slot, volume, volume);
    }

    void GameCubeAudioBackend::ReleaseVoiceState(ActiveVoiceState& voice, bool stopNativeVoice) {
        if (stopNativeVoice) {
