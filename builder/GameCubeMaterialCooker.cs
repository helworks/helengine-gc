...
/// <summary>
/// Translates GameCube material schema payloads into cooked platform-owned runtime material assets.
/// </summary>
public sealed class GameCubeMaterialCooker {
    /// <summary>
    /// Cooks one GameCube material request into a serialized platform-owned material payload.
    /// </summary>
    ...
    public PlatformMaterialCookResult Cook(PlatformMaterialCookRequest request) {
        ...
        } else if (!string.Equals(request.TargetPlatformId, "gamecube", StringComparison.OrdinalIgnoreCase)) {
            throw new InvalidOperationException($"GameCube cannot cook materials for target platform '{request.TargetPlatformId}'.");
        } else if (!IsSupportedSchema(request.SchemaId)) {
            throw new InvalidOperationException($"GameCube does not support material schema '{request.SchemaId}'.");
        }
        ...
        PlatformMaterialAsset cookedAsset = new() {
            ...
            RendererFamilyId = string.IsNullOrWhiteSpace(request.SelectedGraphicsProfileId)
                ? throw new InvalidOperationException("GameCube material cooking requires a graphics profile id.")
                : request.SelectedGraphicsProfileId,
            TextureRelativePath = ResolveTextureRelativePath(request.FieldValues),
            DoubleSided = ResolveBoolean(request.FieldValues, GameCubeMaterialSchemaIds.DoubleSidedFieldId, false),
    ...
    /// <summary>
    /// Returns whether one schema id is supported by the GameCube cooked-material contract.
    /// </summary>
    /// <param name="schemaId">Schema id to evaluate.</param>
    /// <returns>True when the schema id maps to the GameCube cooked-material contract.</returns>
    static bool IsSupportedSchema(string schemaId) {
        return string.Equals(schemaId, GameCubeMaterialSchemaIds.StandardTexturedSchemaId, StringComparison.OrdinalIgnoreCase)
            || string.Equals(schemaId, GameCubeMaterialSchemaIds.StandardShaderSchemaId, StringComparison.OrdinalIgnoreCase);
    }

    /// <summary>
    /// Resolves the optional texture-relative path for one GameCube material request.
    /// </summary>
    ...
    static string ResolveTextureRelativePath(IReadOnlyDictionary<string, string> fieldValues) {
        ...
        } else if (fieldValues.TryGetValue(GameCubeMaterialSchemaIds.TextureRelativePathFieldId, out string textureRelativePath)
            && !string.IsNullOrWhiteSpace(textureRelativePath)) {
            return textureRelativePath;
        } else if (fieldValues.TryGetValue(GameCubeMaterialSchemaIds.TextureIdFieldId, out string textureId)
    ...
    static bool ResolveVertexColorMode(IReadOnlyDictionary<string, string> fieldValues) {
        ...
        if (fieldValues == null) {
            throw new ArgumentNullException(nameof(fieldValues));
        } else if (!fieldValues.TryGetValue(GameCubeMaterialSchemaIds.VertexColorModeFieldId, out value)
        ...
        throw new InvalidOperationException($"GameCube vertex color mode '{value}' is not supported.");
    }
    ...
    static bool ResolveLightingMode(IReadOnlyDictionary<string, string> fieldValues) {
        ...
        if (fieldValues == null) {
            throw new ArgumentNullException(nameof(fieldValues));
        } else if (!fieldValues.TryGetValue(GameCubeMaterialSchemaIds.LightingModeFieldId, out value)
            || string.IsNullOrWhiteSpace(value)) {
            return true;
        ...
        throw new InvalidOperationException($"GameCube lighting mode '{value}' is not supported.");
    }
    ...
    static void ResolveBaseColor(
        ...
        if (fieldValues == null) {
            throw new ArgumentNullException(nameof(fieldValues));
        } else if (!fieldValues.TryGetValue(GameCubeMaterialSchemaIds.BaseColorFieldId, out value)
            || string.IsNullOrWhiteSpace(value)) {
            red = 255;
            green = 255;
            blue = 255;
    ...
    static void ParseColor(string value, out byte red, out byte green, out byte blue, out byte alpha) {
        if (string.IsNullOrWhiteSpace(value)) {
            throw new InvalidOperationException("GameCube base color must be provided.");
        }
        ...
        if (normalizedValue.Length == 6) {
            ...
            throw new InvalidOperationException($"GameCube color '{value}' must use #RRGGBB or #RRGGBBAA format.");
        }
    ...
    static bool ResolveBoolean(IReadOnlyDictionary<string, string> fieldValues, string fieldId, bool defaultValue) {
        ...
        throw new InvalidOperationException($"GameCube boolean field '{fieldId}' value '{value}' is invalid.");
    }
}
