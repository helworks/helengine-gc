...
/// <summary>
/// Cooks shared-engine texture assets into GameCube-native serialized texture payloads.
/// </summary>
public sealed class GameCubeTextureCooker {
    ...
    /// <param name="settings">Resolved GameCube texture cook settings.</param>
    /// <returns>GameCube-native cooked texture asset.</returns>
    public TextureAsset CookTexture(TextureAsset sourceTexture, GameCubeTextureCookSettings settings) {
        ...
        if (workingTexture.ColorFormat == TextureAssetColorFormat.GxRgb5A3) {
            ...
            throw new InvalidOperationException($"GameCube texture cooking requires RGBA32 source textures, but got '{workingTexture.ColorFormat}'.");
        }
    ...
    /// <summary>
    /// Encodes one logical RGBA32 texture into tiled GameCube GX RGB5A3 bytes.
    /// </summary>
    /// <param name="sourceTexture">RGBA32 source texture asset.</param>
    /// <returns>Tiled GameCube-endian RGB5A3 bytes.</returns>
    static byte[] EncodeRgba32ToGxRgb5A3(TextureAsset sourceTexture) {
        if (sourceTexture == null) {
            throw new ArgumentNullException(nameof(sourceTexture));
        } else if (sourceTexture.Colors == null) {
            throw new InvalidOperationException("GameCube texture cooking requires RGBA32 color data.");
        }
        ...
        if (sourceTexture.Colors.Length != expectedColorByteCount) {
            throw new InvalidOperationException("GameCube texture cooking requires tightly packed RGBA32 source bytes.");
        }
